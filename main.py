...
def load_data():
    ...
    try:
        ...
        df = df[df.iloc[:, 0].astype(str).str.contains("Employee") == False]
...
if st.session_state.df is not None:
    ...
    with st.sidebar:
        ...
        with st.form("input_form"):
            ...
            names = sorted([str(n) for n in df[name_col].unique()])
                ...
                if delta <= 0:
                    ...
                    # Maternity、Sick、Jury 等其他所有假期直接操作原列
                        ...
                        if t_col:
                            ...
                            # 如果是统计列(Used/Leave)，则加上天数
                            else:
                                df.loc[idx, t_col] += val

                    # 记录到日历
                    if mode == "请假 (扣除)":
                        new_days = [pd.DataFrame([[ (start_d + timedelta(days=i)).strftime("%Y-%m-%d"), name, tp, 1.0 ]], columns=["Date", "Name", "Type", "Days"]) for i in range(delta)]
                        st.session_state.tracking = pd.concat([st.session_state.tracking] + new_days, ignore_index=True)
                    
                    st.session_state.df = df
                    st.success(f"已处理 {name} 的 {tp} 申请")
                    st.rerun()
